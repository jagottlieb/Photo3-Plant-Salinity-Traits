...
class LentiscPistacia(object):
	...
	GWMAX = 0.002 #.002   May need to consider this
	GPMAX = 7.5 #
...
class SaltySoilGW(SoilGW):
	...
	def psi_s(self, s): 
		...
		return self.PSI_SS*(s**-self.B) - self.cs*R*self.IV*self.TS*10.**(-6.)
	...
	def psi_s_osm(self, s):
		...
		return -self.cs*R*self.IV*self.TS*10.**(-6.)
	def psi_gw(self, gw_cs):
		...
		return self.PSI_SS - gw_cs*R*self.IV*self.GWTS*10.**(-6.) - self.GW_Depth*g*RHO_W*10**(-6)
	...
	def psi_gw_osm(self, gw_cs):
		...
		return gw_cs*R*self.IV*self.GWTS*10.**(-6.)
...
class HalophyteGW(Hydro):
	...
	GWTS = 293 # groundwater temp (K)
	IV = 2. # van't hoff coefficient for NaCl
	psi_w_i = -1.3244 #-2.5 #-1.3244 #-2.5 #-1.3244 #Initial xylem water potential when w = 0.86
	TL = 293 #Assumed water temperature for Vant Hoff
	...
	def __init__(self, species, atm, soil, photo, vwi, cw):
		...
		self.delta_cw = 0 #Change in xylem concentration from starting value
		self.delta_psi_w = 0
		self.w = self.vw/self.VWT
		self.psi_w_0 = self.psi_w_i/(1-1/(self.w**(1/400)))#3512 # Chuang et al. constant for plant water storage in MPa
		#self.psi_x_i = -0.5 #initialize value
		...
		self.out = least_squares(self.fBalinit, (-1, 292.),args= (soil, photo, atm.phi, atm.ta, atm.qa, photo.cm, soil.s, self.lai, self.gp, 1., self.zr, self.psi_wf(self.vw, self.cw), soil.gw_cs), method = 'trf', ftol = 3e-16, xtol= 3e-16, x_scale='jac', max_nfev=2000) #least squares solver
		self.psi_l = self.out.x[0]
		print(self.psi_l)
		self.tl = self.out.x[1]
		print(self.tl)
		self.psi_x_i = self.psi_x(self.ev, self.psi_l, self.gp, self.lai)
	...
	def update(self, atm, soil, photo, dt):
		...
		self.psi_x_i = self.delta_psi_w#self.psi_x(self.ev, self.psi_l, self.gp, self.lai) + self.delta_psi_w
		self.uptake_a.append(self.uptake(self.E, self.gp,self.psi_x_i, self.psi_b(self.vw, self.ev, self.psi_l, self.psi_wf(self.vw, self.cw), self.gp, self.gwf(self.psi_wf(self.vw, self.cw)), self.lai, self.cw, soil), self.lai, soil.gw_cs))
		self.cw = (self.MW/self.vw) 
		self.delta_cw = (self.MW - self.MW1)/self.vw
		self.delta_psi_w = -self.delta_cw*R*self.IV*self.TL*10.**(-6.)
		self.psi_x_i = self.delta_psi_w
		self.w = self.vw/self.VWT
		self.gp = self.gpf(self.psi_l)
		self.out = least_squares(self.fBal, (-1, 292.),args= (soil, photo, atm.phi, atm.ta, atm.qa, photo.cm, soil.s, self.lai, self.gp, 1., self.zr, self.psi_wf(self.vw, self.cw), soil.gw_cs, self.psi_x_i), method ='trf', ftol = 3e-16, xtol = 3e-16, x_scale ='jac', max_nfev=2000)
		self.psi_l = self.out.x[0]
		self.tl = self.out.x[1]
		#print(self.out.fun)
		#print(self.tl)
		...
		self.water_balance.append(self.evf(photo, atm.phi, atm.ta, self.psi_l, atm.qa, self.tl, photo.cm, self.lai, 1.) - self.qbx(self.gp,self.psi_x_i, self.psi_b(self.vw,self.evf(photo, atm.phi, atm.ta, self.psi_l, atm.qa, self.tl, photo.cm, self.lai, 1.), self.psi_l, self.psi_wf(self.vw,self.cw),self.gp,self.gwf(self.psi_wf(self.vw,self.cw)), self.lai,self.cw, soil), self.lai) - self.qwf(self.vw,self.evf(photo, atm.phi, atm.ta, self.psi_l, atm.qa, self.tl, photo.cm, self.lai, 1.), self.gp,self.psi_l, self.lai, self.cw,dt))
		self.ev = self.evf(photo, atm.phi, atm.ta, self.psi_l, atm.qa, self.tl, photo.cm, self.lai, 1.)
		...
		self.delta_psi_w_a.append(self.delta_psi_w)
		...
		self.psi_x_a.append(self.psi_x_i)
		self.qw_a.append(self.qwf(self.vw, self.ev, self.gp, self.psi_l, self.lai, self.cw, dt))
		self.qbx_a.append(self.qbx(self.gp, self.psi_x_i, self.psi_b(self.vw, self.ev, self.psi_l, self.psi_wf(self.vw, self.cw),self.gp, self.gwf(self.psi_wf(self.vw, self.cw)), self.lai,self.cw, soil), self.lai))
...
# 	    psi_wf_osm_ft1 = -(self.MW1/self.VWT)*R*self.IV*TL*10.**(-6.) #Osmotic Potential at full turgor using initial cw
# 	    eta = 27.7 #Modulus of Elasticity for Lentiscus following Christofferson, 2016
# #(vw/self.VWT-self.D1)**self.D2 - cw*R*self.IV*TL*10.**(-6.) + 0.5*self.Plant_h*g*RHO_W*10**(-6) #Turgor pressure minus osmotic potential in the storage plus gravitational potential at 0.5 plant height
	    return self.psi_w_0 - (self.psi_w_0/self.w**(1/400)) - self.delta_cw*R*self.IV*TL*10.**(-6.) + 0.5*self.Plant_h*g*RHO_W*10**(-6)
	def psi_wf_turgor(self, vw, cw): 
	    ...
	    psi_wf_osm_ft1 = -(self.MW1/self.VWT)*R*self.IV*TL*10.**(-6.)
	    return max(-psi_wf_osm_ft1 - eta*((1-(vw/self.VWT))/(1-aF)),0) #(vw/self.VWT-self.D1)**self.D2
	...
	def psi_b(self, vw, ev, psi_l, psi_w, gp, gw, lai, cw, soil):
	    return (self.e(gp,lai)*self.psi_x_i + self.a(self.lai, soil.psi_s(soil.s), soil.psi_gw(soil.gw_cs), self.gwf(self.psi_wf(self.vw, self.cw)), self.psi_wf(self.vw, self.cw), soil, soil.s, self.zr, psi_l))/(self.e(gp,lai)+self.d(soil, soil.s,self.zr, psi_l)) #Update psi_b - it shouldn't be lower than psi_l at peak transpiration...
	...
	def gsr(self, soil, s, zr, psi_l): # Using the formulation from Huang et al., 2016
	    ...
	    B = 4500*0.9 #Soil default fraction of 0.88 1000 used in Vogel paper #root length density: root length per unit soil volume (m/m^3)
	...
	def ggwr(self, soil, s, zr, psi_l): #Using the formulation from Huang et al., 2016
	    ...
	    B = 4500*0.1 # GW Fraction default of 0.12 4000 #root length density: root length per unit soil volume (m/m^3)
	    kr = 10**-8 #10**-9 #root permeability
	...
	def uptake(self, E, gp, _i, psi_b, lai, c_gw):
	    if self.Salt_Uptake == True:
		    return self.qbx(gp, self.psi_x_i, psi_b, lai)*c_gw*(1-E)*30*60*10**(-6)
		    #return self.qwf(self.vw, self.ev, self.gp, self.psi_l, self.lai, self.cw, dt)*c_gw*(1-E)*30*60*10**(-6)
		    print(self.qbx(gp, self.psi_x_i, psi_b, lai)*c_gw*(1-E)*30*60)
	    else:
		    return 0
		    print("0")
	def fBal(self, params, soil, photo, phi, ta, qa, c1, s, lai, gp, ared, zr, psi_w, gw_cs, psi_x): 
	    psi_l, tl = params
	    if lai < 1.: # assumes only a portion of solar radiation is absorbed by crops
	    	...
	    	return (phi - self.shf(tl, ta, lai) - LAMBDA_W*RHO_W*self.evf(photo, phi, ta, psi_l, qa, tl, c1, lai, ared)/1000000., self.evf(photo, phi, ta, psi_l, qa, tl, c1, lai, ared) - self.qbx(gp,psi_x, self.psi_b(self.vw,self.evf(photo, phi, ta, psi_l, qa, tl, c1, lai, ared),psi_l,psi_w,gp,self.gwf(psi_w),lai,self.cw,soil),lai) - self.qwf(self.vw,self.evf(photo, phi, ta, psi_l, qa, tl, c1, lai, ared),gp,psi_l,lai, self.cw,dt))
		    	#self.evf(photo, phi, ta, psi_l, qa, tl, c1, lai, ared) \
				#-((self.a(self.lai, soil.psi_s(s), soil.psi_gw(soil.gw_cs), self.gwf(self.psi_wf(self.vw, self.cw)), self.psi_wf(self.vw, self.cw), soil, soil.s, self.zr) + \
	...
	def fBalinit(self, params, soil, photo, phi, ta, qa, c1, s, lai, gp, ared, zr, psi_w, gw_cs): 
	    psi_l, tl = params
	    if lai < 1.: # assumes only a portion of solar radiation is absorbed by crops
		    #return (phi*lai - self.shf(tl, ta, lai) - LAMBDA_W*RHO_W*self.evf(photo, phi, ta, psi_l, qa, tl, c1, lai, ared)/1000000., self.evf(photo, phi, ta, psi_l, qa, tl, c1, lai, ared)-self.qbx(gp,self.psi_x(self.evf(photo, phi, ta, psi_l, qa, tl, c1, lai, ared),psi_l,gp,lai),self.psi_b(self.vw,self.evf(photo, phi, ta, psi_l, qa, tl, c1, lai, ared),psi_l,psi_w,gp,self.gwf(psi_w),lai,self.cw,soil),lai) - self.qwf(self.vw,self.evf(photo, phi, ta, psi_l, qa, tl, c1, lai, ared),gp,psi_l,lai, self.cw,dt))\
		    return (self.evf(photo, phi, ta, psi_l, qa, tl, c1, lai, ared)-self.qbx(gp,self.psi_x(self.evf(photo, phi, ta, psi_l, qa, tl, c1, lai, ared),psi_l,gp,lai),self.psi_b(self.vw,self.evf(photo, phi, ta, psi_l, qa, tl, c1, lai, ared),psi_l,psi_w,gp,self.gwf(psi_w),lai,self.cw,soil),lai) - self.qwf(self.vw,self.evf(photo, phi, ta, psi_l, qa, tl, c1, lai, ared),gp,psi_l,lai, self.cw,dt), phi*lai - self.shf(tl, ta, lai) - LAMBDA_W*RHO_W*self.evf(photo, phi, ta, psi_l, qa, tl, c1, lai, ared)/1000000.)
	...
	def leaf_solver(self, ta, lai, phi, photo, qa, cx, gp, ared, vw, cw, dt, soil): #Bespoke solver function to test whether fsolve is creating error in water balance
		tl_env_len = 100 #Length of tl envelope
		psi_l_env_len = 1000 #Length of psi_l envelope
		tl_env = np.linspace(289,294,tl_env_len) #Envelope of possible tl values
		psi_l_env = np.linspace(-3.5,-0.3,psi_l_env_len) #Envelope of possible psi_l values
		print(psi_l_env)
		best_diff = 0.1 #Initialize a level for best difference value which updates as better approximations are found
		start = time.time() #Check function runtime
		tl_out = 294 #Default if no better approximation is found
		psi_l_out = -3 #Default if no better approximaton is found
		for tl_i in tl_env:
			ev_i = (phi - self.shf(tl_i, ta, lai))*1000000/(LAMBDA_W*RHO_W)
			diff_env =[] #Envelope of difference between Ev constrained by energy balance and Ev(tl,psi_l)
			for psi_l_i in psi_l_env:
				diff_env.append(ev_i - self.qbx(gp, self.psi_x(self.evf(photo, phi, ta, psi_l_i, qa, tl_i, cx, lai, ared),psi_l_i,gp,lai), self.psi_b(vw,self.evf(photo, phi, ta, psi_l_i, qa, tl_i, cx, lai, ared),psi_l_i, self.psi_wf(vw, cw), gp, self.gwf(self.psi_wf(vw, cw)),lai, cw, soil), lai) - self.qwf(vw, self.evf(photo, phi, ta, psi_l_i, qa, tl_i, cx, lai, ared), gp, psi_l_i, lai, cw, dt)) #Append difference between ev_i and internal water flux for each psi_l value in psi_l_env
			min_diff = min(diff_env, key=abs) #Smallest difference in Ev values
			#print(diff_env)
			min_index = diff_env.index(min_diff) #Index of minimum difference
			print(min_index)
			psi_l = psi_l_env[min_index] #psi_l value which produces the best approximation for the given tl value
			if abs(min_diff)<=best_diff: #Check if best approximation for tl, psi_l pair is better than previous iterations
				#print("good value")
				best_diff = min_diff
				tl_out = tl_i #set tl output to new best approximation
				psi_l_out = psi_l #set psi_l output to new best approximation
				#print(psi_l_out)

		end = time.time()
		#print(end-start)
		return (psi_l_out, tl_out)
