# -*- coding: utf-8 -*-
"""
Created on Sun Oct 23 19:14:36 2022
...
class LentiscPistacia(object):
	...
	GWMAX = 0.07 #0.002#0.002 #.002   May need to consider this
	GPMAX = 7.5 #
	VWT = 0.012 #0.012 #0.012 #0.0096*LAI #0.000249 #
	CAP = 0.15
	VCMAX0 = 37 #57.7
	JMAX0 = 60 #98.5
...
class SaltySoilGW(SoilGW):
	...
	def __init__(self, stype, dynamics, zr, s, cs, cgw, gw_z, FE): #gw_z = depth to wt, gw_cs = salt concentration in gw, S_y = specific yield
...
class HalophyteGW(Hydro):
	...
	GWTS = 293 # groundwater temp (K)
	IV = 2. # van't hoff coefficient for NaCl
	psi_w_i = -1.0 #-2.5 #-1.3244 #-2.5 #-1.3244 #Initial xylem water potential when w = 0.86
	TL = 293 #Assumed water temperature for Vant Hoff


	def __init__(self, species, atm, soil, photo, vwi, cw, FE):
		...
		self.psi_w_0 = self.psi_w_i/(1-1/(self.w**(1/400)))
	...
	def ggwr(self, soil, s, zr, psi_l): #Using the formulation from Huang et al., 2016
	    ...
	    B = 1500*0.1#4500*0.1 # GW Fraction default of 0.12 4000 #root length density: root length per unit soil volume (m/m^3)
	    kr = 10**-8 #10**-9 #root permeability
