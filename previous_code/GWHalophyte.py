...
class PhotoVico(object):
	...
	def k2(self, phi, tl, ared):###
		...
		return self.k1(phi,tl)*self.a2()/self.v_cmax(tl,ared)
	def a2(self, tl, phi):###
	...
	def a_phigsTl(self, gs, rd, k1, k2):
	    """Net Photsynthetic Demand for CO2 (Vico,2013) (umol/(m^2s^1))"""
	    return 0.5*(k1 +(k2+self.ca)*gs - rd - np.sqrt((k1 + (k2-self.ca)*gs - rd)^2 - 4*gs*(-self.ca*gs*k2 - k2*rd - k1*self.gamma(tl)))) #Equation 1
	...
	def gscv(self, phi, ta, psi_l, qa, tl, cx, ared, photo,c1,lai, **kwargs):
	    """Stomatal conductance (Vico 2013 Formulation) to CO2, per unit leaf area (mol/m2/s)"""
	    if self.a_phigsTl(gs, rd, k1, k2) < 0.:
	        return 0.
	    else:
	        #return self.A1*self.an(phi, psi_l, tl, cx, ared, **kwargs)/self.ca*self.fD(VPD(ta, qa))
		    return P_ATM*hydro.evf(photo, phi, ta, psi_l, qa, tl, c1, lai, ared)/(self.A1*(hydro.qi(tl,psi_l)-atm.qa)) #Equation 2
...
class SaltySoilGW(SoilGW):
	...
	def update(self, dt, zr, qs, qgw):
		...
		self.gw_z = self.gw_z - dt*60*self.leak(self.s)/10**6 + qgw*60*dt/10**6 ###specificyield not taken into account 
...
class HalophyteGW(Hydro):
	...
	def __init__(self, species, atm, soil, photo, vwi, cw, lam_s1, lam_gw1):
		...
		self.psi_l, self.tl = fsolve(self.fBal, (-0.5, 290.), args= (soil, photo, atm.phi, atm.ta, atm.qa, photo.cm, soil.s, self.lai, self.gp, 1., self.zr, self.psi_wf(self.vw, self.cw), soil.gw_cs))
	...
	def update(self, atm, soil, photo, dt):
		self.psi_l, self.tl = fsolve(self.fBal, (-1., 290.), args= (soil, photo, atm.phi, atm.ta, atm.qa, photo.cm, soil.s, self.lai, self.gp, 1., self.zr, self.psi_wf(self.vw, self.cw), soil.gw_cs))
		self.ev = self.evf(photo, atm.phi, atm.ta, self.psi_l, atm.qa, self.tl, photo.cx, self.lai, 1.)
		self.gp = self.gpf(self.psi_l)
	...
	def output(self):
		return {'psi_l': self.psi_l_a, 'psi_w': self.psi_w_a, 'gp': self.gp_a, 'gsv': self.gsv_a, 'tl': self.tl_a, 'ev': self.ev_a, 'vw': self.vw_a, 'cw': self.cw_a, 'qgw': self.qgw_a, 'qs': self.qs_a, 'psi_b': self.psi_b_a, 'psi_x': self.psi_x_a, 'qw': self.qw_a, 'qbx': self.qbx_a, 'gs': self.gs_a, 'gwf': self.gwf_a, 'ggwr': self.ggwr_a, 'gsr': self.gsr_a}
