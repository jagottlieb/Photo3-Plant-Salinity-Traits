...
class HalophyteGW(Hydro):
	...
	def gsr(self, soil, s, zr, psi_l): # Using the formulation from Huang et al., 2016
	    ...
	    B = SR #Soil default fraction of 0.88 1000 used in Vogel paper #root length density: root length per unit soil volume (m/m^3)
	...
	def ggwr(self, soil, s, zr, psi_l): #Using the formulation from Huang et al., 2016
	    ...
	    B = GWR # GW Fraction default of 0.12 4000 #root length density: root length per unit soil volume (m/m^3)
	    kr = 10**-8 #10**-9 #root permeability
