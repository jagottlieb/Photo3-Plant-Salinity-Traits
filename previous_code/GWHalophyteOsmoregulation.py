# -*- coding: utf-8 -*-
"""
Created on Sat Oct 29 21:41:42 2022
...
class LentiscPistacia(object):
	...
	GWMAX = 0.2 #.002   May need to consider this
	GPMAX = 7.5 #
...
class HalophyteGW(Hydro):
	...
	def psi_wf(self, vw, cw): 
	    ...
	    psi_wf_osm = -cw*R*self.IV*TL*10.**(-6.) #Osmotic Potential at full turgor
	    eta = 27.7 #Modulus of Elasticity for Lentiscus following Christofferson, 2016
	    #print(psi_wf_osm_ft)
	    return psi_wf_osm + max(-psi_wf_osm - eta*((1-vw/self.VWT)/(1-aF)),0) + self.F_CAP*self.Plant_h*g*RHO_W*10**(-6) #(vw/self.VWT-self.D1)**self.D2 - cw*R*self.IV*TL*10.**(-6.) + 0.5*self.Plant_h*g*RHO_W*10**(-6)#Turgor pressure minus osmotic potential in the storage plus gravitational potential at 0.5 plant height
	def psi_wf_turgor(self, vw, cw): 
	    """Turgor pressure of stored water (MPa)"""
	    TL = 293.
	    eta = 27.7
	    aF = 0.75
	    psi_wf_osm = -cw*R*self.IV*TL*10.**(-6.)
	    return max(-psi_wf_osm - eta*(1-(vw/self.VWT)),0) #(vw/self.VWT-self.D1)**self.D2
	def psi_wf_osm(self, vw, cw):
	    """Osmotic potential of stored water (MPa)"""
	    TL = 293.
	    aF = 0.75
	    psi_wf_osm = -cw*R*self.IV*TL*10.**(-6.)
	    return psi_wf_osm #-cw*R*self.IV*TL*10.**(-6.)
